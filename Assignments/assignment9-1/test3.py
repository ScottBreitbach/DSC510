...
def main():
    ...
    nameFile = input('Please enter a file name for export: ')
    if nameFile[-4:] == '.txt':         # in case user adds file extension
        fileName = nameFile
    else:
    ...
    if regex.search(fileName) == None:
        ...
        print('Special characters [@_!#$%^&*()<>?/\|}{~:] not accepted.\n'
              'Please try again.')
    process_file(wordCounts, fileName)  # exports data to file
    print('Data exported to file:', fileName)
