...
def byCity(city):
    '''constructs url parameters for city input'''
...
def byCityState(city, state):
    '''constructs url parameters for city, state input'''
...
def byZip(zip):
    '''constructs url parameteres for zip code input'''
    print("Getting", zip, "...")
    querystring = {"zip":zip,
                   "appid":appid,
                   "units":units}
...
def req(querystring, cityZip):
    '''assemble url and request data from openweathermap site'''
...
def prettyPrint(resString, state):
    '''print weather information for user'''
...
def moreDetail(resString):
    '''provide user additional weather details if requested'''
...
def windDirection(wind_dir):
    '''returns cardinal direction from azimuth direction'''
    if wind_dir <= 22.5:
        direction = 'N'
    elif wind_dir <= 67.5:
        direction = 'NE'
...
def allDone():
    '''provides user option to exit or continue'''
...
def main():
    '''requests city or zip from user; calls the other functions'''
    state = ""
